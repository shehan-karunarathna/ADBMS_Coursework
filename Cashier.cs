...
namespace KSmarketing_system
{
    public partial class Cashier : Form
    {
        ...
        public void GetCartTotal()
        {
            ...
            double delivery_fee = 1500;
            double newtotal= sales + delivery_fee;
            
            //double vatable = sales - vat;

            lblVat.Text = delivery_fee.ToString("#,##0.00");
            //lblVatable.Text = vatable.ToString("#,##0.00");
            lblDisplayTotal.Text = newtotal.ToString("#,##0.00");
        ...
        private void txtBarcode_TextChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    int _qty;
                    cn.Open();
                    cm = new SqlCommand("SELECT * FROM tbProduct WHERE barcode LIKE '" + txtBarcode.Text + "' or pcode LIKE '" + txtBarcode.Text + "'", cn);
                    dr = cm.ExecuteReader();
                    dr.Read();
                    if (dr.HasRows)
                    {
        ...
        public void AddToCart(string _pcode, double _price, int _qty)
        {
            try
            {
                ...
                cn.Open();
                //procedure 3
                cm = new SqlCommand("GetCartData", cn);
                ...
                else
                {
                    ...
                    // procedure 1 to insert the cart items
                    cn.Open();
                    cm = new SqlCommand("InsertCartItem", cn);
                    cm.CommandType = CommandType.StoredProcedure;
                    cm.Parameters.AddWithValue("@transno", lblTranNo.Text);
                    cm.Parameters.AddWithValue("@pcode", _pcode);
